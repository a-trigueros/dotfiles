...
elseif (Get-Command exa -ErrorAction SilentlyContinue) {
    function ls { exa --icons --group-directories-first $args }
    function ll { exa --icons --group-directories-first -l $args }
    function la { exa --icons --group-directories-first -la $args }
    function lt { exa --icons --group-directories-first --tree $args }
}
else {
...
function Extract-Archive {
    ...
    switch ($extension) {
        '.zip' { Expand-Archive -Path $Path -DestinationPath (Get-Location) }
        '.tar' { tar -xf $Path }
        '.gz' { tar -xzf $Path }
        '.tgz' { tar -xzf $Path }
        '.bz2' { tar -xjf $Path }
...
function explorer { Start-Process explorer . }
